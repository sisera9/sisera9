...
function buildProfileLines(projects) {
  const shortNames = {
    "Hitung HPP Website": "Hitung HPP"
  };

  return [
    { type: "header", value: "sisera@build" },
    { type: "row", key: "Name", value: "Siti Serli Aviva" },
    { type: "row", key: "Role", value: "Student & Developer" },
    { type: "row", key: "School", value: "SMKN 1 Probolinggo" },
    { type: "row", key: "Based", value: "Probolinggo, Indonesia" },
    { type: "blank" },
    { type: "section", value: "LEARN.FOCUS" },
    { type: "row", key: "WebDev", value: "Responsive & Modern UI" },
    { type: "row", key: "Frontend", value: "HTML, CSS & JavaScript" },
    { type: "row", key: "Tools", value: "Git, VS Code & Vercel" },
    { type: "blank" },
    ...
    { type: "blank" },
    { type: "footer", value: "LEARNING, BUILDING, AND GROWING" }
  ];
}

// TEMA WAKTU/WARNA: HACKER BLUE CYBERPUNK
const palettes = {
  dark: {
    backgroundStart: "#050B14",
    backgroundEnd: "#0A1220",
    panel: "#0F172A",
    primary: "#E0F2FE",
    muted: "#64748B",
    cyan: "#00F0FF",      // Cyan neon khas hacker
    blue: "#38BDF8",      // Electric Blue
    violet: "#818CF8",    // Cyber Violet Accent
    green: "#2DD4BF",     // Cyber Mint Green
    red: "#00F0FF",       // Indicator accent
    portraitStart: "#00F0FF",
    portraitEnd: "#0284C7",
    scanBlend: "screen"
  },
  light: {
...
function buildAmbientPortraitLayer(layout, colors, size) {
  ...
  <path d="M ${left} ${(centerY + 42).toFixed(1)} C ${(left + 32).toFixed(1)} ${(centerY + 8).toFixed(1)}, ${(centerX - orbitWidth * 0.3).toFixed(1)} ${(centerY + 58).toFixed(1)}, ${(centerX - orbitWidth * 0.19).toFixed(1)} ${(centerY + 27).toFixed(1)}" fill="none" stroke="${colors.blue}" stroke-width="1" opacity="0.2"/>
  <path d="M ${right} ${(centerY - 52).toFixed(1)} C ${(right - 38).toFixed(1)} ${(centerY - 18).toFixed(1)}, ${(centerX + orbitWidth * 0.31).toFixed(1)} ${(centerY - 70).toFixed(1)}, ${(centerX + orbitWidth * 0.2).toFixed(1)} ${(centerY - 30).toFixed(1)}" fill="none" stroke="${colors.green}" stroke-width="1" opacity="0.2"/>
  <g fill="${colors.cyan}">
    ...
    <circle cx="${right - (isDesktop ? 10 : 16)}" cy="${(centerY - 58).toFixed(1)}" r="1.7" opacity="0.4"/>
  </g>
</g>`;
}
...
function buildSystemLayer({ x, y, lineHeight, fontSize }, colors, profileLines) {
  ...
  profileLines.forEach((line, index) => {
    ...
    if (line.type === "header") {
      rows.push(`<text x="${x}" y="${lineY}" class="system-head"><tspan fill="${colors.cyan}">${escapeXml(line.value)}</tspan><tspan fill="${colors.muted}"> ------------------------------------------</tspan></text>`);
      return;
    }
...
<title id="title">Siti Serli Aviva - Student &amp; Developer</title>
<desc id="description">A hacker-themed profile card with Sisera's ASCII portrait, learning focus, and selected work.</desc>
<defs>
  ...
  <linearGradient id="border" x1="0" y1="0" x2="1" y2="0"><stop offset="0" stop-color="${colors.cyan}"/><stop offset="0.48" stop-color="${colors.blue}"/><stop offset="1" stop-color="${colors.green}"/></linearGradient>
  <linearGradient id="scan" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="${colors.cyan}" stop-opacity="0"/><stop offset="0.5" stop-color="${colors.cyan}" stop-opacity="0.46"/><stop offset="1" stop-color="${colors.blue}" stop-opacity="0"/></linearGradient>
  <radialGradient id="portrait-halo"><stop offset="0" stop-color="${colors.cyan}" stop-opacity="0.25"/><stop offset="0.48" stop-color="${colors.blue}" stop-opacity="0.1"/><stop offset="1" stop-color="${colors.backgroundStart}" stop-opacity="0"/></radialGradient>
  ...
  <pattern id="portrait-grid" width="44" height="44" patternUnits="userSpaceOnUse"><path d="M 44 0 H 0 V 44" fill="none" stroke="${colors.blue}" stroke-width="0.65" opacity="0.15"/><circle cx="0" cy="0" r="1.2" fill="${colors.cyan}" opacity="0.3"/></pattern>
  ...
  <style>
    ...
    .panel-title { font-family: 'Courier New', Consolas, monospace; font-size: ${isDesktop ? 11 : 12}px; letter-spacing: 2px; fill: ${colors.cyan}; opacity: 0.9; }
    ...
    .live-label { font-family: 'Courier New', Consolas, monospace; font-size: ${isDesktop ? 10 : 9}px; letter-spacing: 1px; fill: ${colors.cyan}; }
...
<circle cx="${titlebar.x + 39}" cy="${titlebar.y + titlebar.height / 2}" r="5" fill="${colors.blue}" opacity="0.7"/>
<circle cx="${titlebar.x + 57}" cy="${titlebar.y + titlebar.height / 2}" r="5" fill="${colors.green}" opacity="0.78"/>
<text x="${titleCenter}" y="${titlebar.y + titlebar.height / 2 + 5}" text-anchor="middle" class="terminal-label">sisera@build ~ % ./profile</text>
${isDesktop ? `<circle cx="${liveX}" cy="${titlebar.y + titlebar.height / 2}" r="4" fill="${colors.cyan}"/><text x="${liveX + 10}" y="${titlebar.y + titlebar.height / 2 + 4}" class="live-label">ONLINE</text>` : ""}
<rect x="${visual.x}" y="${visual.y}" width="${visual.width}" height="${visual.height}" rx="${visual.radius}" fill="${colors.panel}" fill-opacity="0.38" stroke="url(#border)" stroke-opacity="0.5"/>
<rect x="${info.x}" y="${info.y}" width="${info.width}" height="${info.height}" rx="${info.radius}" fill="${colors.panel}" fill-opacity="0.42" stroke="url(#border)" stroke-opacity="0.5"/>
<text x="${layout.visualTitle.x}" y="${layout.visualTitle.y}" class="panel-title">PORTRAIT / SISERA</text>
<text x="${layout.infoTitle.x}" y="${layout.infoTitle.y}" class="panel-title">PROFILE / DEVELOPER</text>
${ambientPortrait}
...
${system}
<text x="${layout.width / 2}" y="${layout.footerY}" text-anchor="middle" class="mono" font-size="10" letter-spacing="1.5" fill="${colors.muted}">WEB DEVELOPMENT / FRONTEND SYSTEMS / SMKN 1 PROBOLINGGO</text>
...
<rect x="3" y="3" width="${layout.width - 6}" height="${layout.height - 6}" rx="${layout.outerRadius - 2}" fill="none" stroke="url(#border)" stroke-width="2" opacity="0.8"/>
</svg>`;
}
...
async function main() {
  ...
  if (!Array.isArray(projects) || projects.length === 0) {
    throw new Error("Featured project data must contain at least one project.");
  }
  ...
  console.log(`Generated hacker-blue hero assets from ${basename(sourcePath)}.`);
}
